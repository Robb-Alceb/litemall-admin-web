...
<!--        <el-form-item :label="$t('Business_Days')" prop="weeks">
        ...
        <el-form-item :label="$t('可订阅门店')" prop="selectShopIds">
          <el-select v-model="subscribe.selectShopIds" multiple clearable filterable style="width: 400px;">
            <el-option v-for="item in shopOps" :value="item.id" :label="item.name"></el-option>
          </el-select>
        </el-form-item>
    ...
    <el-row style="padding-top: 20px;">
      <el-button type="primary" size="mini" @click="handleUpdateSubscirbe()">{{$t('确定')}}</el-button>
...
<script>
  ...
  import { readSubscribe, updateSubscribe } from '@/api/subscribe'
  export default {
    name: "edit",
    data(){
      return {
    ...
    created(){
      this.subscribe.id =  this.$route.query.id;
      this.getSubscribe();
      this.getGoodsOps();
      this.getShopOps();
    },
    methods: {
      getSubscribe(){
        readSubscribe(this.subscribe.id).then(response=>{
          if(response.data.data.subscribeShopDtos.length > 0){
            response.data.data.selectShopIds = response.data.data.subscribeShopDtos.map(item=>{
              return item.shopId
            })
          }
      ...
      handleUpdateSubscirbe(){
        this.$refs['subscribeForm'].validate((valid) => {
          if(valid){
            ...
            updateSubscribe(this.subscribe).then(response => {
              this.$notify.success({
